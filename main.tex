...
\caption{Coefficients and ANOVA of the final general linear model. Note that both the predictor variables and the response variable are transformed non-linearly, as discussed in the text. That is why some coefficients may seem counterintuitive at first -- e.g. a negative slope for \protect\varr{engine capacity} with respect to \protect\varr{co2}. $\lambda_j$'s taken from \cref{table:predictorvar_transform}.}
\label{table:lm}
\end{table}
...
In the exploratory analysis, we noted the strong pairwise correlations between \varr{urban metric}, \varr{extra urban metric}, \varr{combined metric}, and \varr{co2}. This points to the problem of multicollinearity. Indeed, when we do more formal tests, we find strong evidence for multicollinearity in the data: the variance inflation factors (VIF) of the mentioned variables are all larger than 100, and the mean VIF of 456 is significantly larger than 1. The condition number $ \sqrt{\lambda_{max} / \lambda_{min}} = 129 $ is greater than 30. In the preceding linear regression, we have ignored this multicollinearity. In the following exercise however, we will use a biased method with a smaller variance as a remedy.
